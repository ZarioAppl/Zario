...
export default function ZarioApp({ supabase, initialSession }) {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      if (dly.data?.length) setDE(dly.data.map(e => ({ id: e.id, desc: e.description || "", amount: e.amount, time: e.entry_time || "", date: e.entry_date })));
  ...
  const dailyTotal = dailyEntries.filter(e => e.date === todayISO() || !e.date).reduce((s, e) => s + Number(e.amount), 0);
  ...
  // For monthly mode: balance also deducts paid bills
  const balance = incomeType === 0 ? curIncome - curExp - paidBillsTotal : curIncome - curExp;
  const savRate = curIncome > 0 ? Math.round((balance / curIncome) * 100) : 0;
  // Bills paid this month reduce available balance
  const paidBillsTotal = bills.filter(b => b.paid).reduce((s, b) => s + Number(b.amount), 0);
  ...
  const addDaily = async (entry) => {
    ...
    const row = { id: crypto.randomUUID(), ...entry, time, date: todayISO() };
    setDE(p => [row, ...p]);
    const uid = await getUID();
  ...
  const Daily = () => (
    <div>
      ...
      <Card st={{ marginBottom: 16 }}>
        ...
        <GoalRef dg={dailyGoal} wg={weeklyGoal} mg={monthlyGoal} dark={dark}
      ...
      <Card>
        <CT right={<span style={{ fontSize: 13, color: C.muted }}>{dailyEntries.length} registros</span>}>Historial de Ingresos</CT>
            ...
            return (
              <div key={date} style={{ marginBottom: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, paddingBottom: 6, borderBottom: "1px solid " + C.border }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: "0.5px" }}>📅 {fmtD(date)}</span>
                  <span style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 13, fontWeight: 700, color: "#10B981" }}>+{fmt(dayTotal)}</span>
                </div>
                {entries.map(e => (
                  <div key={e.id} className="row-item" style={{ marginBottom: 6 }}>
          ...
          return (
            <div key={date} style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, paddingBottom: 6, borderBottom: `1px solid ${C.border}` }}>
                ...
                <span style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 13, fontWeight: 700, color: "#F43F5E" }}>-{fmt(dayTotal)}</span>
              </div>
              {exps.map(x => (
                <div key={x.id} className="row-item" style={{ marginBottom: 6 }}>
                  ...
                  <div style={{ flex: 1, marginLeft: 10, overflow: "hidden" }}>
                    ...
                    <div style={{ fontSize: 11, color: C.muted }}>{x.category}</div>
                  </div>
